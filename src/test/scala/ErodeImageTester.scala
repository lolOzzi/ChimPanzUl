...
class ErodeImageTester(dut: CPUTop) extends PeekPokeTester(dut) {
  //Do not run the CPU
  poke(dut.io.run, 0)
  ...
  while(running) {
    ...
    //System.out.println("Does write? " + peek(dut.io.dataWriteEnableTest))
    //System.out.println("Data Memory Adress: " + peek(dut.io.addressTest))
    //System.out.println("Data To Write: " + peek(dut.io.dataWriteTest) + "\n")
...
object ErodeImageTester {
  def main(args: Array[String]): Unit = {
    println("Testing the Erode Image")
    iotesters.Driver.execute(
      Array("--generate-vcd-output", "on",
        "--target-dir", "generated",
        "--top-name", "ErodeImageTester"),
      () => new CPUTop()) {
      c => new ErodeImageTester(c)
    }
  }
}
